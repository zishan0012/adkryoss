...
const ServicesTabs = () => {
  ...
  useLayoutEffect(() => {
    const ctx = gsap.context(() => {
      ...
      // ----- PARALLAX (reduced / disabled on mobile) -----
      if (!isMobile) {
      ...
      // ----- TABS POP OUT (simpler on mobile) -----
      gsap.fromTo(tabButtonsRef.current,
        {
          opacity: 0,
          y: isMobile ? 30 : 50,
          scale: 0.9,
          rotation: !isMobile ? (i) => (i % 2 === 0 ? -3 : 3) : 0,
        },
        {
          opacity: 1,
          y: 0,
          ...
          stagger: isMobile ? 0.1 : { amount: 1.5, from: "start" },
          ease: isMobile ? "back.out(1)" : "elastic.out(1, 0.6)",
          scrollTrigger: {
      ...
      gsap.fromTo(contentBoxRef.current,
        { opacity: 0, y: isMobile ? 40 : 60, scale: 0.97 },
        {
          opacity: 1,
          y: 0,
          scale: 1,
      ...
      // ----- CONTENT TEXT (initial state set for later tab change) -----
      gsap.set([contentTitleRef.current, contentDescRef.current, contentImageRef.current], {
        opacity: 1,
        x: 0,
        scale: 1,
      });
  ...
  // ----- TAB CHANGE ANIMATION (lightweight) -----
  useLayoutEffect(() => {
  ...
  // ----- HOVER EFFECT (only on buttons) -----
  const handleTabHover = (e, isEnter) => {
    gsap.to(e.target, {
      scale: isEnter ? 1.05 : 1,
      y: isEnter ? -2 : 0,
      boxShadow: isEnter ? "0 10px 20px -5px rgba(0,102,204,0.4)" : "0 4px 6px -2px rgba(0,0,0,0.05)",
      duration: 0.3,
      ease: "power2.out",
    });
  };
